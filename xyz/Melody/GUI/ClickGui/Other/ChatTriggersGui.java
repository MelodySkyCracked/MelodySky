...
public class ChatTriggersGui {
    ...
    private TextField messageField = new TextField(1, "message", 0, 0, 130, 20);
    private DickGui dick;

    public ChatTriggersGui(DickGui dickGui) {
    ...
    public void draw(int n, int n2, int n3, int n4) {
        ...
        RenderUtil.drawFastRoundedRect(n5 + 15, n2 + 5, n3 - 20, n2 + 180, 5.0f, new Color(100, 100, 100, 80).getRGB());
        FontLoaders.NMSL20.drawString("Target: " + this.selectedTarget, n5 + 30, n2 + 35, -1);
        CTObject cTObject = Client.getCTObject(this.selectedTarget);
        FontLoaders.NMSL20.drawString("Message: " + (cTObject != null ? cTObject.getMessage() : ""), n5 + 30, n2 + 52, -1);
        this.targetField.drawTextBox(n5 + 30, n2 + 70);
        this.messageField.drawTextBox(n5 + 30, n2 + 100);
        FontLoaders.NMSL20.drawString("Strip: ", n5 + 35, n2 + 132, -1);
        RenderUtil.outlineRect(n5 + 140, n2 + 128, n5 + 155, n2 + 143, new Color(20, 20, 20, 80).getRGB(), 10.0f, 5.0f);
        if (cTObject != null) {
            FontLoaders.NMSL20.drawString(cTObject.isStrip() ? "Enabled" : "Disabled", n5 + 63, n2 + 132, -1);
            RenderUtil.drawFastRoundedRect(n5 + 142, n2 + 130, n5 + 153, n2 + 141, 10.0f, cTObject.isStrip() ? new Color(90, 220, 120, 130).getRGB() : new Color(255, 60, 60, 130).getRGB());
        }
        RenderUtil.drawFastRoundedRect(n5 + 30, n2 + 150, n5 + 90, n2 + 170, 5.0f, new Color(20, 20, 20, 80).getRGB());
        FontLoaders.NMSL20.drawString("Add", n5 + 50, n2 + 157, -1);
        RenderUtil.drawFastRoundedRect(n5 + 100, n2 + 150, n5 + 160, n2 + 170, 5.0f, new Color(20, 20, 20, 80).getRGB());
        FontLoaders.NMSL20.drawString("Remove", n5 + 113, n2 + 157, -1);
        ...
        if (Client.chatTriggerMap.isEmpty()) {
            ...
            for (CTObject cTObject2 : Client.chatTriggerMap) {
                ...
                FontLoaders.NMSL20.drawString(EnumChatFormatting.GRAY + "Strip " + (cTObject2.isStrip() ? EnumChatFormatting.GREEN + "Enabled" : EnumChatFormatting.RED + "Disabled"), n5 - 80, n2 + 15 + n7, -1);
                n7 += 20;
            }
        }
        Scissor.end();
    ...
    public void onMouseClick(double d, double d2, int n) {
        ...
        if (d > (double)this.startX && d < (double)f) {
            object = Client.chatTriggerMap.iterator();
            while (object.hasNext()) {
                CTObject cTObject = (CTObject)object.next();
                if (d2 > (double)(this.startY + 8.0f + (float)n2) && d2 < (double)(this.startY + 8.0f + (float)n2 + 20.0f) && d2 > (double)(this.startY + 5.0f)) {
                    this.selectedTarget = cTObject.getTarget();
                }
                n2 += 20;
            }
        }
        ...
        if (this.selectedTarget != null && d > (double)(f + 140.0f) && d < (double)(f + 155.0f) && d2 > (double)(this.startY + 128.0f) && d2 < (double)(this.startY + 143.0f) && (object = Client.getCTObject(this.selectedTarget)) != null) {
            ((CTObject)object).setStrip(!((CTObject)object).isStrip());
        }
        if (d2 > (double)(this.startY + 150.0f) && d2 < (double)(this.startY + 170.0f)) {
            if (d > (double)(f + 30.0f) && d < (double)(f + 90.0f)) {
                if (this.targetField.func_146179_b().equals("") || this.messageField.func_146179_b().equals("")) {
                    NotificationPublisher.queue("Warn", "Target/Message should not be empty.", NotificationType.ERROR, 5000);
                    this.targetField.func_146180_a("");
                    return;
                }
                Client.chatTriggerMap.add(new CTObject(this.targetField.func_146179_b(), this.messageField.func_146179_b(), false));
                NotificationPublisher.queue("Success", "Added " + this.targetField.func_146179_b() + " to your chattriggers list.", NotificationType.SUCCESS, 5000);
                this.selectedTarget = this.targetField.func_146179_b();
                this.targetField.func_146180_a("");
                ...
                Client.chatTriggerMap.remove(Client.getCTObject(this.selectedTarget));
    ...
    public void handleMouseScroll(float f, float f2) {
        ...
        if (this.mouseWheel < 0 && this.vertical < (float)(Client.chatTriggerMap.size() - 1)) {
            this.vertical -= 10.0f;
