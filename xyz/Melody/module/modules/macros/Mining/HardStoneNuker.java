...
public class HardStoneNuker
extends Module {
    private ArrayList broken = new ArrayList();
    private BlockPos closestStone;
    private int ticks = 0;
    private TimerUtil timer = new TimerUtil();
    private Option areaCheck = new Option("Crystal Hollows", true);
    private Option ores = new Option("Ores", true);
    private Numbers range = new Numbers("Range", 3.8, 2.0, 6.0, 0.1);
    private Numbers height = new Numbers("Height", 2.0, 0.0, 3.0, 0.5);
    ...
    public void onTick(EventRender2D eventRender2D) {
        if (((Boolean)this.areaCheck.getValue()).booleanValue() && Client.instance.sbArea.getCurrentArea() != Areas.Crystal_Hollows) {
    ...
    private BlockPos closestStone() {
        ...
        Vec3i vec3i = new Vec3i((double)f, ((Double)this.height.getValue()).doubleValue(), (double)f);
        Vec3i vec3i2 = new Vec3i((double)f, 1.0, (double)f);
